...
def grade_func(file,step=0,outp=sys.stdout):
    ...
    testcase = [('smith.john@megacorp.com','john'),
                ('WHILOW.BRANDON@megacorp.com','BRANDON'),
                ('maggie.white@mompop.net','maggie'),
                ('Bob.Bird@mompop.net','Bob'),
                ('BB.King@mompop.net','BB'),
                ('a.b@megacorp.com','b'),
                ('a.b@mompop.net','a'),
                ('A.B@megacorp.com','B'),
                ('A.B@mompop.net','A'),
                ('cummings.e.e@consultant.biz','e'),
                ('McDougal.Raymond.Clay@consultant.biz','Raymond'),
                ('A.B.C@consultant.biz','B')]
    if not step:
        testcase = testcase[:-3]
    ...
    for item in ast.walk(body):
        if type(item) == ast.If:
            for sub in item.orelse:
                if type(sub) == ast.If and len(item.orelse) == 1:
                    found1 = True
                else:
                    found2 = True
    
    if not found1:
        outp.write('Function %s does not have an elif-statement in it.\n' % repr(function))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT,max(0,score))
    
    if not found2:
        outp.write('Function %s does not have an else-statement in it.\n' % repr(function))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT,max(0,score))
    
    if text.count(' else:') > 1:
        outp.write('Function %s has two many else-statements (make sure you are using elif).\n' % repr(function))
        score -= 0.25
        if not step:
            return (FAIL_INCORRECT,max(0,score))
...
def grade_file(file,outp=sys.stdout):
    ...
    if not status:
        ...
        if not status:
           status, p2b = grade_structure(file,2,outp)
        else:
            p2b = 0
        p2 = 0.7*p2a+0.3*p2b
        if p2 == 1:
