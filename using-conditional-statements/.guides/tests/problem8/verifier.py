...
WORKSPACE = [os.path.expanduser('~'),'workspace','exercise8']
#WORKSPACE = []
...
class TestPlan(object):
    ...
    def first1(self,s):
        ...
        """
        if not 'first_vowel' in self._tests:
            self._tests['first_vowel'] = []
        self._tests['first_vowel'].append(s)
        return None
        ...
        if test:
            ...
            if step < 2:
                ...
                if step:
                    ...
                    func.first_vowel = testplan.first2
                else:
                    func.pigify = testplan.piggy1
                    func.first_vowel = testplan.first1
...
def grade_script(file,step=0,outp=sys.stdout):
    ...
    for item in code.body:
        if type(item) == ast.If:
            script = item
        elif type(item) in [ast.Import, ast.FunctionDef]:
            pass
    ...
    good = True
    if script is None:
        outp.write('There is no if-statement at the end of the script %s.\n' % repr(file))
        return (FAIL_INCORRECT,0)
    else:
        comp = script.test
        good = type(comp) == ast.Compare
        good = good and type(comp.left) == ast.Name and comp.left.id == '__name__'
        good = good and len(comp.ops) == 1 and type(comp.ops[0]) == ast.Eq
        good = good and len(comp.comparators) == 1 and type(comp.comparators[0]) == ast.Str
        good = good and comp.comparators[0].s == '__main__'
        if not good:
            group = text[comp.lineno-1][comp.col_offset:]
            group = group.strip()[:-1]
            outp.write('The if-test %s is not what you were directed to use.\n' % repr(group))
            score -= 0.2
            if not step:
                return (FAIL_INCORRECT,max(0,score))
        
        for item in extras:
            outp.write('File %s has extraneous code at line %d:\n' % (repr(file),item.lineno))
    ...
    if tests.asserted != {}:
        outp.write("You have not put all of the script code in %s under an if.\n" % repr(file))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT, max(0,score))
    
    if 'Module funcs passed all tests.' in env.printed:
        outp.write("You have not put the final print statement in %s under an if.\n" % repr(file))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT, max(0,score))
    ...
    tests.reset()
    if not env.execute():
        outp.write("The script code in %s crashed." % repr(file))
        outp.write('\n'.join(env.printed))
        return (FAIL_CRASHES,0)
    
    for func in ['first_vowel','pigify']:
        if not func in tests.tested:
            if good:
                outp.write("The script code in %s does not call the 'test_%s'.\n" % (repr(file),func))
            else:
                outp.write("The script code in %s is not executing 'test_%s'.\n" % (repr(file),func))
            score -= 0.4
            if not step:
                return (FAIL_BAD_STYLE, max(0,score))
    ...
    additional = None
    for line in env.printed:
        if not line in ['___test___','Testing first_vowel()','Testing pigify()','Module funcs passed all tests.']:
            additional = line
    ...
    if not additional is None:
        ...
        if step < 2:
            return (FAIL_BAD_STYLE, max(0,score))
    elif good and not 'Module funcs passed all tests.' in env.printed:
        outp.write("You have removed the final print statement.\n")
        score -= 0.3
        if not step:
            return (FAIL_BAD_STYLE, max(0,score))
    elif good and env.printed[-1] != 'Module funcs passed all tests.':
...
def grade_file(file,outp=sys.stdout):
    ...
    status, p1 = grade_testcases(file,1,outp)
    if p1 == 1:
        outp.write('The test cases look unaffected.\n\n')
    else:
        outp.write('\n')
    
    if not status:
        outp.write('Script code comments:\n')
        status, p2 = grade_script(file,1,outp)
        if p2 == 1:
