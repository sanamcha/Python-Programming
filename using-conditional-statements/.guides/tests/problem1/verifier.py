...
def grade_func(file,step=0,outp=sys.stdout):
    ...
    The step parameter is the phase in the grading pass.  Step 0 checks that it works
    for the letter 'a'.  Step 1 checks that it works for the letter 'e'.  Steps 2
    checks that it works for the remaining non-'y' vowels.  Steps 3 and 4 check that
    it works for all vowels.  Steps 0-3 will stop at the first error found.  Otherwise 
    ...
    if step > 2 and len(env.printed) != 0:
...
def grade_file(file,outp=sys.stdout):
    ...
    if not status:
        ...
        status, p2 = grade_func(file,4,outp)
        if p2 == 1:
