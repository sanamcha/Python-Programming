...
WORKSPACE = [os.path.expanduser('~'),'workspace','exercise7']
#WORKSPACE = []
...
def grade_military(file,step=0,outp=sys.stdout):
    """
    Returns the test result and score for the bug fixes to time_to_military.
    
    The step parameter is the phase in the grading pass.  Step 0 will verify that the
    first (according to the test script) bug has been fixed.  Steps 1 and up will verify
    for all bugs. Step 2 verfies that all print statements have been removed. Steps 0-2 
    will stop at the first error found.  Otherwise it will continue through and try to 
    find all errors.
    ...
    function = 'time_to_military'
    testcase = [('12:00 AM','00:00'),('12:00 PM','12:00'),('1:10 PM','13:10'),
                ('8:27 PM','20:27'),('9:05 PM','21:05'),('11:15 PM','23:15'),
                ('1:10 AM','01:10'),('8:27 AM','08:27'),('9:05 AM','09:05'),
                ('11:15 AM','11:15'),('10:30 AM','10:30'),('10:30 PM','22:30')]
    if not step:
        testcase = testcase[:-2]
    ...
    if step > 1 and len(env.printed) != 0:
...
def grade_file(file,outp=sys.stdout):
    ...
    Precondition: outp is a stream writer
    """
    outp.write("Comments on 'time_to_military':\n")
    status, p1 = grade_military(file,3,outp)
    if p1 == 1:
    ...
    if not status:
        ...
        status, p2 = grade_minutes(file,3,outp)
        if p2 == 1:
            outp.write("The function looks good.\n\n")
        else:
            outp.write('\n')
