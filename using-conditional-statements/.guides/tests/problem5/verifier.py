...
WORKSPACE = [os.path.expanduser('~'),'workspace','exercise5']
#WORKSPACE = []
...
def commafy(pos):
    """
    Returns a curried function for turning a number into a string with a comma.
    
    The function returned takes a single argument x.  It converts it to a string and
    places a comma at position pos (from the left). If pos is out of range, no comma 
    is added. This is a helper for generating a lot of test cases.
    
    Paramater pos: the position to place the comma
    Precondition: pos is an int >= 0
    """
    def result(x):
        value = str(x)
        if pos <= len(value):
            value = value[:len(value)-pos]+','+value[len(value)-pos:]
        return value
    return result
...
def grade_func(file,step=0,outp=sys.stdout):
    ...
    goods = list('0123456789')
    goods.extend(map(str,random.sample(range(10,100),10)))
    goods.extend(map(str,random.sample(range(100,1000),10)))
    goods.extend(map(commafy(3),random.sample(range(1000,10000),10)))
    goods.extend(map(commafy(3),random.sample(range(10000,100000),10)))
    goods.extend(map(commafy(3),random.sample(range(100000,1000000),10)))
    ...
    bads.extend(map(lambda x : '0'+x,goods))
    for x in range(7):
        if x < 2:
            bads.extend(map(commafy(x),random.sample(range(1,10),2)))
        if x < 3:
            bads.extend(map(commafy(x),random.sample(range(10,100),2)))
        if x < 4:
            bads.extend(map(commafy(x),random.sample(range(100,1000),2)))
        if x != 3:
            bads.extend(map(commafy(x),random.sample(range(1000,10000),2)))
            bads.extend(map(commafy(x),random.sample(range(10000,100000),2)))
            bads.extend(map(commafy(x),random.sample(range(100000,1000000),2)))

    testcase = list(map(lambda x : (x,True),goods))+list(map(lambda x : (x,False),bads))
