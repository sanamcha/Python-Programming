...
class RecursiveEnvironment(object):
    """
    Instance is an execution environment for safe recursion.
    ...
    def __init__(self,name,path=None,base=0):
        """
        Initializes the execution evironment
    ...
    def _analyze(self, code):
        ...
        try:
            ...
            for item in data.body:
                if type(item) == ast.FunctionDef and len(item.body) > 1:
                    found.append(item.body[1])
            
            variable = '__depthguard__'
            for pos in range(len(found)):
                item = found[pos]
                offset = item.lineno-1+pos*4
                indent = item.col_offset
                text.insert(offset,(' '*(indent+4))+('return %s' % repr(self._base)))
                text.insert(offset,(' '*indent)+('if %s <= 0:' % variable))
                text.insert(offset,(' '*indent)+('%s -= 1' % variable))
                text.insert(offset,(' '*indent)+('global %s' % variable))
            
            text.insert(0,'%s = %d' % (variable, self.LIMIT))
            return '\n'.join(text)
        except:
            import traceback
            traceback.print_exc()
    ...
    def reset(self,main=False):
        ...
        the module.
        """
        self._mods.__depthguard__ = self.LIMIT
        self._main = main
        self._prints = []
        self._inputs = []
        self._errors = False
