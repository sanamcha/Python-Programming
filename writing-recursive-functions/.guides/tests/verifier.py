...
import builtins
import ast
from modlib import RecursiveEnvironment
...
WORKSPACE = [os.path.expanduser('~'),'workspace','exercise']
#WORKSPACE = []
...
def import_module(name,base=0):
    ...
    try:
        ...
        environment = RecursiveEnvironment(refs,WORKSPACE,base)
...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    function = 'hanoi'
    testcase = range(1,10)
    testcase = tuple(map(lambda x : (x,round(2**x-1)), testcase))
    ...
    elif not hasattr(env.module,'__depthguard__'):
    ...
    for pair in testcase:
        try:
            ...
            if env.module.__depthguard__ <= 0:
                outp.write("The call %s(%s) encountered an infinite recursion.\n" % (function,repr(pair[0])))
                outp.write("Stopped recursion after %d calls.\n" % (env.LIMIT-env.module.__depthguard__))
...
def grade_func2(file,step=0,outp=sys.stdout):
    ...
    function = 'lucas'
    testcase = [(0,1,-1,0),(1,1,-1,1),(2,1,-1,1),(6,1,-1,8),
                (6,2,-1,70),(6,1,1,0),(6,1,2,5),(8,-3,2,-255),
                (9,2,-1,985),(10,-1,1,1)]
    ...
    for pair in testcase:
        try:
            env.module.__depthguard__ = env.LIMIT
            expected = pair[-1]
            received = func(*pair[:-1])
            if env.module.__depthguard__ <= 0:
                outp.write("The call %s%s encountered an infinite recursion.\n" % (function,repr(pair[:-1])))
                outp.write("Stopped recursion after %d calls.\n" % (env.LIMIT-env.module.__depthguard__))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
            elif received != expected:
                outp.write("The call %s%s returns %s, not %s.\n" % (function, repr(pair[:-1]), repr(received), repr(expected)))
                score -= 1/len(testcase)
                if not step:
                    return (FAIL_INCORRECT,max(0,score))
...
def grade_structure(file,func,step=0,outp=sys.stdout):
    ...
    if not found3:
        outp.write('Function %s does not contain a recursive call.\n' % repr(func))
        score -= 0.5
        if not step:
            return (FAIL_INCORRECT,max(0,score))
