...
def grade_func(file,step=0,outp=sys.stdout):
    ...
    elif not hasattr(env.module,'count_while'):
    ...
    for inps in testcase:
        try:
            ...
            outp.write('Your code has exceeded the limit of while-loop iterations.\n')
            outp.write('Check to see if your code contains an infinite while-loop.')
            score -= 1/len(testcase)
            if not step:
                return (FAIL_INCORRECT,max(0,score))
        except:
