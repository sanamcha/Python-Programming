...
class WhileEnvironment(object):
    ...
    def _analyze(self, code):
        ...
        # Code to create generator that allows up to 10,000 while-loop iterations
        preamble = ["# ----------------------AUTOGRADER CODE--------------------- #",
                    "# Define 'loop_generator' to prevent infinite while-loops",
                    "def loop_generator(n):",
                    "    for x in range(n):",
                    "        yield x\n",
                    "# Create generator that crashes on 10,001st loop iteration",
                    "count_while = loop_generator(10000)", 
                    "# ---------------------------------------------------------- #\n"]
        ...
        try:
            ...
            # Insert 'next(count)' on the first line of the body of each while-loop.
            # This increments the generator to keep track of while-loop iterations.
            for pos in range(len(found)):
                item = found[pos][0]
                offset = item.lineno + pos
                indent = item.body[0].col_offset
                text.insert(offset,' '*indent + 'next(count_while)  # AUTOGRADER CODE')
            
            # Find line number (func_lineno) of the first function definition
            found_funcDef = []
            for item in ast.walk(data):
                if type(item) == ast.FunctionDef:
                    found_funcDef.append((item,item.lineno))
            first_funcDef = found_funcDef[0][0]
            func_lineno = first_funcDef.lineno
            
            # Insert generator preamble before the first function defintion
            index = 0
            for lineno in range(func_lineno, func_lineno + len(preamble)):
                text.insert(lineno-1, preamble[index])
                index += 1
