        ...
        try:
            ...
            if env.inputed == []:
                ...
                if len(values[1:]) > 1 and env.printed == []:
                    outp.write("The function call '%s(%d)' did not print error messages when the autograder made bets in the following tuple: %s \n" % (function, args, repr(values[1:-1]))) 
                    outp.write("Check your code to make sure the proper error message prints under the conditions specified in the docstring of the %s function.\n" % repr(function))
                    score -= 0.5
                    if not step:
                        return (FAIL_INCORRECT,max(0,score))
                elif len(values[1:]) > 1:
