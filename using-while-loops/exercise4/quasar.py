...
def session(bet):
    ...
    while score < 20:
        choice = prompt("Choose (a) 4-7, (b) 1-8, or (s)top: ", ('a', 'b', 's'))
        if choice == 's':
            break
        if choice == 'a':
    ...
    if winnings >= 0:
        ...
        print(f"You lost {-winnings} credits.")
...
def play(credits):
    ...
    while True:
        ...
        # Get a bet and play a session
        bet = get_bet(credits)
        payout = session(bet)
        credits += payout

        # to fix debug, immediately show updated credits after session
        print(f"You have {credits} credits.")
        ...
        # Ask to continue or payout
        choice = prompt("Do you want to (c)ontinue or (p)ayout? ", ('c', 'p'))
        if choice == 'p':
            print(f"You leave with {credits} credits.")
            return
